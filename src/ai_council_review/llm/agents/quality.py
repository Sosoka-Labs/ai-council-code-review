...
def build_quality_executor(
    ...
    """Build an AgentExecutor for the quality agent.
...
def run_quality_agent(
    ...
    executor = build_quality_executor(config, browser)
    ...
    try:
        ...
        findings = parse_findings(result["output"], agent_name="quality", logger=logger)
        logger.info("Quality agent finished", findings=len(findings))
        if not findings:
            logger.info("Quality agent raw output", raw_output=result["output"])
        return findings
