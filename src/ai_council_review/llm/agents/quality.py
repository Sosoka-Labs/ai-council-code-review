...
def build_quality_chain(
    ...
    if registry is not None:
        skills = resolve_skills_for_agent("quality", config, registry)
        if skills:
            prompt = apply_skills(prompt, skills)
            skill_mode = "bodies"
    chain = prompt | llm.with_structured_output(FindingList)
    return bind_chain_metadata(chain, "quality", skills, skill_mode)
...
def run_quality_agent(
    ...
    chain = build_quality_chain(config, registry=registry)
