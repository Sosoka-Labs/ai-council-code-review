"""Tests for ai_council_review.github_client retry behavior."""
...
class TestGitHubClientRetry:
    ...
    def _create_client(self) -> GitHubClient:
        ...
        with patch("ai_council_review.github_client.Github") as mock_github:
    ...
    def test_retry_with_jitter(self) -> None:
        ...
        with (
            patch(
                "ai_council_review.github_client.requests.request",
                side_effect=responses,
            ) as mock_request,
            patch("ai_council_review.github_client.time.sleep") as mock_sleep,
            patch(
                "ai_council_review.github_client.random.uniform",
                return_value=0.5,
            ),
        ):
